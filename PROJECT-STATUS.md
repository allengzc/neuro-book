...
部署侧已补充 Docker Compose 与免 Docker native 单机生产方案：默认 `app + SQLite`，运行时使用 `workspace/`，SQLite 数据文件默认位于 `workspace/.nbook/neuro-book.sqlite`；根目录 `.env` 保存运行环境变量，根目录 `config.yaml` 收敛为 Boot Config 示例/启动期配置，并通过 `${DATABASE_KIND}` / `${DATABASE_URL}` 镜像数据库环境变量。数据库解析已统一为 env first、Boot Config fallback、缺省 SQLite；Postgres 仍作为一等部署选项保留：Docker 模式内置 Postgres 使用 `docker-compose.postgres.yml`，外部 Postgres 使用 `docker-compose.external-db.yml`，native 模式只支持 SQLite 或外部 PostgreSQL URL，不负责安装或管理本机 Postgres；不会自动导入旧 Postgres 数据。SQLite 下通过内部 `DatabaseLock` 表在事务内取得写锁，保护 Plot sortOrder 和管理员状态检查这类 read-before-write 窗口。模型 Provider、API Key、Agent 默认 profile、UI/editor 偏好已迁入 Global Config `workspace/.nbook/config.json`，Project 覆盖写入 `workspace/{project}/.nbook/config.json`。这些运行时文件都不进 Git，真实文件不再作为仓库文件维护，主线历史已移除曾提交过的真实 `config.yaml`，已暴露过的模型 Provider token 仍应视为泄露并轮换。本轮进一步把部署入口收敛成 Node CLI + 远程 `npx` 交互脚本，并补齐 `config.example.yaml` 的 Boot Config 注释化说明；针对低内存服务器 Nuxt build OOM，已补充本地 GHCR 发布脚本和 release-only GitHub Actions 镜像发布路径，且 `neuro-book-deploy` 支持 `ghcr`、`source` 与 `native` 三种模式，`.deploy/` 在 Docker 模式保留 compose override，在 native 模式生成本地说明和 `start-native` / `create-admin-native` 脚本并清理旧 compose override；GHCR 现在发布 `neuro-book-runtime` 基础工具镜像和 `neuro-book` 开箱即用 app 镜像，两者都携带 Node.js、Python 3、ripgrep、git、bash、ca-certificates、coreutils 和 findutils 等 agent 常用工具；source 模式使用本地 `Dockerfile.source-runtime` 构建同规格 runtime，不依赖 GHCR；native 模式会检测宿主机 `node`、`npm`、`git`、`bun`、`rg`、Unix `bash/coreutils/findutils` 并交互确认安装缺失工具，Windows 可在 `winget`、Scoop 或自动选择之间切换，构建迁移后生成启动脚本并打印 `node .output/server/index.mjs` 启动命令，不生成 systemd/pm2；部署开局不再询问或默认写入模型 Provider / API Key，相关配置留给前端设置页或 `workspace/.nbook/config.json`；`scripts/deploy.mjs` 已改为 `arch` 开发服务器 source 模式快速同步入口，通过 SSH 拉取最新代码、宿主机构建并用一次性 sudo 校验重启 app 容器；`docs/operator-bridge.md` 已作为连接开发者、用户和用户 Agent 的交付与运维桥梁，集中索引部署执行步骤、敏感信息边界和关键项目文档；release-only 镜像工作流已补充 OCI metadata、buildx cache 和 Canary prerelease 流程说明，并显式启用 Node 24 action runtime；同时 `auth.enabled` 已迁入 Global Config，默认开启，关闭时整站与管理员接口都退化为无遮罩访问。

数据库运行时已切到 SQLite-first 双库结构：`server/database/*` 统一解析 env-first + Boot Config fallback，Prisma 使用 SQLite/Postgres 双 schema 与双 migrations，`StoryThread.tags` 以 JSON array 文本持久化并在 repository 层对外保持 `string[]`，`execute_sql` 根据当前 Database Kind 动态切换 description、schema summary 和 SQL adapter。SQLite 下新增内部 `DatabaseLock` 表，用事务内写入锁表的方式保护 Plot sortOrder 和管理员状态检查等 read-before-write 窗口；当前 dev `.env` 的 Postgres 配置仍会被 `bun run generate` / migrate 脚本优先读取，不会被缺省 SQLite 覆盖。
...
| Module | Status | Notes |
| --- | --- | --- |
| Workspace | Active | 已转向按 novel 隔离，术语以 [spec/workspace/TERMS.md](spec/workspace/TERMS.md) 为准：Workspace Root 默认 `workspace/`，Project Workspace 默认 `workspace/{project}/`。内容节点模板和新小说目录模板已切到 Bundled Workspace Template `assets/workspace/.nbook/templates/...`，用户覆盖层是 `workspace/.nbook/templates/...`；manuscript 新小说模板已使用 `manuscript/001-volume/001-chapter/` 的 volume / chapter 层级；文件树和干净编辑器会响应真实文件变化，网页/磁盘同时修改时走保存冲突对话框。 |
...
- 推进 Project identity migration：删除 `Novel` 表，将公开合同和运行时锚点从 numeric `novelId` 迁到 `projectSlug` / `projectPath`。

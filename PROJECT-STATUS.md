...
- Agent 变量系统后续补齐 `client.*` 前端 apply/ack 管线和 Workspace Root / Project 变量 definition `.compiled` 运行加载；当前已完成 `ctx.vars`、invoke `clientState`、变量工具、session `variable_patch`、profile 内 `session.*` 变量定义注册、DSL `<Variable>` / `<VariableSchema>`，`leader.default` 的 Project Workspace 与 Plot Focus 提醒已内联为普通 `Reminder`。

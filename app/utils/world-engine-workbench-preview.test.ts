...
describe("World Engine Workbench preview redesign", () => {
    it("保留 mock 数据源和三栏 preview 入口", async () => {
        ...
        expect(editor).toContain("buildIssueExplanation");
        ...
        expect(editor).toContain("A1（提醒）");
        expect(editor).toContain("A2（提醒）");
        expect(editor).toContain("E1（持久）");
        expect(editor).toContain("E2（持久）");
        expect(editor).toContain("broken-relative");
        expect(editor).toContain("dangling-ref");

...
`AGENTS.md` 是项目的全局 Agent 指令，对所有 profile 可见。这里放置跨 profile 的通用规则、项目整体风格要求、禁忌事项等。与 `agent-context/{profile}/` 不同，这里的内容不按 profile 隔离。
...
- AGENTS.md vs agent-context 边界：[reference/agent/profile-context-memory.md](../reference/agent/profile-context-memory.md)

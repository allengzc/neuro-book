...
> **API 速览（Task 67 硬切，本文示例已对齐）**：World Engine = Zod schema + 2 工具。详细迁移见 `reference/world-engine/api-migration-zod.md`。
> - Schema: `world-engine/schema/index.ts`（TypeScript + Zod，不再用 `schema.yaml`）
> - 工具: `execute_world_query`（只读 CodeAct）+ `write_world_slice`（写入，patches + JSON Pointer path + 4-op）
> - Subject 创建：首次 `write_world_slice` 在 patch 上声明 `type` 即自动注册，无需单独 create 步骤
...
- 本 skill 使用 World Engine 新 2工具API（`execute_world_query` + `write_world_slice`）维护世界状态，**不引导用户用 plot / simulator / director / emulation 来追踪世界状态**——在写作模式里这些当做不存在。
...
schema 定好后，通过 `write_world_slice` 写入第一批 subject。**首次写入某个 subject 时会自动创建**，不需要单独注册步骤。
...
write_world_slice(projectPath, {
    ...
    patches: [
        ...
        { subjectId: "world", type: "world", name: "星辉大陆", path: "/name", op: "replace", value: "星辉大陆" },
    ],
})
```
...
继续用 `write_world_slice` 把开局那一刻的状态记进时间线：主角转生到哪、初始位置、初始状态、和谁的关系。一个 slice = 一个时间点 + 一组变更，同一时刻发生的事都放进同一个 slice。

```javascript
write_world_slice(projectPath, {
    time: "星辉历312年 5月5日 14:05",
    title: "薇洛丝转生到无名祭坛",
    patches: [
        ...
        { subjectId: "weiluosi", path: "/events",   op: "append",  value: "在无名祭坛睁开眼，发现自己换了身体" },
    ],
})
```
...
- **同一 instant 只能有一个 slice**。如果这个时间点已经有切面，`write_world_slice` 会冲突报错；换一个相邻时间点写入（本工具没有覆盖/编辑已有切面的能力）。
...
- world subject（纪元锚点）和开局要追踪的角色已通过首次 write_world_slice 创建。

...
推荐用 [`skills`](https://skills.sh) CLI 安装 —— `npx skills add notnotype/llmlint` —— 它会把文件装进对应 Agent 的 skills 目录（如 `.claude/skills/llmlint/` 或 NeuroBook 的 `.nbook/agent/skills/llmlint/`）；也可手动复制整个目录。装好后在 skill 目录运行一次依赖安装（`npm install` / `bun install` / `pnpm install`），Agent 即可按文档流程驱动 CLI。

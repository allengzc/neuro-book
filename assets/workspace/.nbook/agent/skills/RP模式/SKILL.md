...
- 当前 Project Workspace 应存在 `simulation/config.yaml`、`simulation/cast.yaml`、`simulation/simulator.md`、`simulation/writer.md` 和 `simulation/subjects/*`。
...
2. 根据 `cast.yaml` 创建或复用 `rp.actor` 会话；每个 subject 只注入自己的 `subject.md`、`knowledge.md`、`mind.md` 与 `state.md`。
...
6. simulator leader 构造 writer brief，调用 `rp.writer`。
7. 最终只把用户可见正文和必要的简短 GM 提示返回给用户。

## 边界

- `leader.rp` 是 simulator leader，不直接写文件。
- `rp.actor` 是 subject simulator；主扮演阶段不读取完整 `simulation/`、`lorebook/`、`reference/` 或其他 subject 文件。
- `rp.actor` 的旁路可以维护自己的 `knowledge.md` 与 `mind.md`；`state.md` 仍交给 simulator leader / 后续状态系统。

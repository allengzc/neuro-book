...
function resolveDatabaseConfig() {
  ...
  const url = envUrl || (kind === "sqlite" ? DEFAULT_SQLITE_URL : bootUrl);
...
function assertAllowedWatchPath(path2, label) {
  ...
  if (!["ctx.session", "ctx.input", "ctx.runtime", "ctx.workspace"].some((prefix) => path2 === prefix || path2.startsWith(`${prefix}.`))) {
    throw new Error(`${label} \u53EA\u80FD\u4ECE ctx.session\u3001ctx.input\u3001ctx.runtime\u3001ctx.workspace \u5F00\u59CB\uFF1A${path2}`);
  }
}
function readPath(context, path2) {
  const customStatePrefix = "ctx.session.customState.";
  if (path2.startsWith(customStatePrefix)) {
    const customPath = path2.slice(customStatePrefix.length);
  ...
  const rootKey = Object.keys(roots).find((key) => path2 === key || path2.startsWith(`${key}.`));
  if (!rootKey) {
    return void 0;
  }
  const rest = path2 === rootKey ? [] : path2.slice(rootKey.length + 1).split(".");
  ...
  context(ctx) {
    return /* @__PURE__ */ jsxs(ProfilePrompt, { children: [
      ...
      /* @__PURE__ */ jsxs(AppendingSet, { children: [
        /* @__PURE__ */ jsx(Reminder, { id: "user-assets-workspace", watchPath: "ctx.workspace.root", repeatEveryTurns: 20, children: /* @__PURE__ */ jsx(Message, { children: [
          "<system-reminder>",
          "User assets workspace:",
          "- agent profiles/skills should use agent/ under current user-assets cwd; repository path: workspace/.nbook/agent",
          typeof ctx.input.role === "string" && ctx.input.role.trim() ? `Role: ${ctx.input.role.trim()}` : "",

...
var globalForPrisma, requireAdapter, createPrismaClient, usePrismaClient, prisma;
var init_prisma = __esm({
  "server/database/prisma.ts"() {
    ...
    requireAdapter = createRequire(import.meta.url);
    createPrismaClient = () => {
      const config2 = resolveDatabaseConfig();
      const adapter = config2.kind === "sqlite" ? new (requireAdapter("@prisma/adapter-libsql")).PrismaLibSql({ url: config2.url }) : new (requireAdapter("@prisma/adapter-pg")).PrismaPg({ connectionString: config2.url });
...
function PlotFocusReminder(props = {}) {
  return Reminder({
    ...
    watch: (ctx) => ctx.session.customState[PLOT_SELECTION_STATE_KEY] ?? null,
    repeatEveryTurns: props.repeatEveryTurns,
...
function renderVariableSchemaNode(state, node) {
  ...
  const payload = node.detail ? { catalog: result.catalog, schemas: result.schemas, issues: result.issues } : { catalog: result.catalog, issues: result.issues };
  return [
    "<variable-schema>",
...
async function resolveWriterChapterTargets(ctx) {
  ...
  const target = await resolveWriterChapterTarget(ctx.session.novelId ?? null, chapterPath);

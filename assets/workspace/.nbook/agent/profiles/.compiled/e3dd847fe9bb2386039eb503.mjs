...
function normalizeSql(sql2) {
  return sql2.trim().replace(/;+$/g, "").trim();
}
...
function getSqlLeadingKeyword(sql2) {
  return normalizeSql(sql2).match(/^[a-zA-Z]+/)?.[0]?.toLowerCase() ?? "";
}
function isReadSql(sql2) {
  const leadingKeyword = getSqlLeadingKeyword(sql2);
  return leadingKeyword === "select" || leadingKeyword === "with";
}
function detectSqlCommand(sql2) {
  const leadingKeyword = getSqlLeadingKeyword(sql2);
...
function validateExecuteSql(sql2) {
  const normalized = normalizeSql(sql2);
...
function hasSqlStatementSeparator(sql2) {
  let index = 0;
  ...
  while (index < sql2.length) {
    const char = sql2[index] ?? "";
    const nextChar = sql2[index + 1] ?? "";
    if (inLineComment) {
      if (char === "\n" || char === "\r") {
        inLineComment = false;
      }
    ...
    if (dollarQuoteTag) {
      if (sql2.startsWith(dollarQuoteTag, index)) {
    ...
    if (char === "$") {
      const match = /^\$[a-zA-Z_][a-zA-Z0-9_]*\$|^\$\$/.exec(sql2.slice(index));
      if (match) {
...
function shouldRefreshChapterTree(sql2) {
  const normalized = normalizeSql(sql2);
  return /\b(?:insert\s+into|update|delete\s+from)\s+"(?:Chapter|Volume|Novel)"/.test(normalized);
}
function buildAgentSqlErrorMessage(sql2, error) {
...
async function executeSql(sql2) {
  const normalized = normalizeSql(sql2);
...
__export(prismaNamespace_exports, {
  ...
  JsonNull: () => JsonNull2,
  ModelName: () => ModelName,
  NovelScalarFieldEnum: () => NovelScalarFieldEnum,
  ...
  SortOrder: () => SortOrder,
  Sql: () => Sql2,
  StoryPhaseScalarFieldEnum: () => StoryPhaseScalarFieldEnum,
  StoryPlotScalarFieldEnum: () => StoryPlotScalarFieldEnum,
  StoryScalarFieldEnum: () => StoryScalarFieldEnum,
  StorySceneRefScalarFieldEnum: () => StorySceneRefScalarFieldEnum,
  StorySceneScalarFieldEnum: () => StorySceneScalarFieldEnum,
  StoryThreadScalarFieldEnum: () => StoryThreadScalarFieldEnum,
...
var PrismaClientKnownRequestError2, PrismaClientUnknownRequestError2, PrismaClientRustPanicError2, PrismaClientInitializationError2, PrismaClientValidationError2, sql, empty2, join4, raw2, Sql2, Decimal2, getExtensionContext, prismaVersion, NullTypes2, DbNull2, JsonNull2, AnyNull2, ModelName, TransactionIsolationLevel, UserScalarFieldEnum, NovelScalarFieldEnum, StoryScalarFieldEnum, StoryPhaseScalarFieldEnum, StoryThreadScalarFieldEnum, StorySceneScalarFieldEnum, StoryPlotScalarFieldEnum, StorySceneRefScalarFieldEnum, SortOrder, QueryMode, NullsOrder, defineExtension;
...
var init_client = __esm({
  "server/generated/prisma/client.ts"() {
    ...
    init_enums();
    init_enums();
    globalThis["__dirname"] = path5.dirname(fileURLToPath5(import.meta.url));
...
function parseReferenceUri(raw3) {
  const normalizedRaw = raw3.trim();
...
function parseStructuredReferenceTarget(raw3) {
  const normalizedRaw = raw3.trim();
...
function isStructuredWorkspaceReferenceTarget(raw3) {
  if (!raw3 || raw3.startsWith("/") || raw3.startsWith("#")) {
    return false;
  }
  if (WORKSPACE_REFERENCE_SCHEME_PATTERN.test(raw3)) {
    return false;
  }
  return true;
}
function normalizeWorkspaceReferenceTarget(raw3) {
  return raw3.trim().replace(/\\/g, "/");
}
  ...
  "server/plot/repositories/prisma-plot-data.repository.ts"() {
    ...
    PrismaPlotDataRepository = class {
      ...
      async lockPlotOrderBucket(sceneId) {
        await this.prisma.$executeRaw(prismaNamespace_exports.sql`SELECT pg_advisory_xact_lock(${sceneId})`);
      }
      /**
       * 统计 Story 下的 Plot 总数。
       */
...
function resolveWorkspacePath(root, inputPath) {
  const baseRoot = path6.resolve(root);
  const resolved = path6.isAbsolute(inputPath) ? path6.resolve(inputPath) : path6.resolve(baseRoot, inputPath);
  if (resolved !== baseRoot && !resolved.startsWith(`${baseRoot}${path6.sep}`)) {
...
async function buildWorkspaceNode(root, absolutePath, options) {
  ...
  const indexPath = isDirectory2 ? path6.join(absolutePath, "index.md") : null;
  const hasIndex = indexPath ? await pathExists(indexPath) : false;
  const contentDirectoryNode = isDirectory2 && hasIndex;
  const contentIndexNode = !isDirectory2 && path6.basename(relativePath).toLowerCase() === "index.md";
...
async function readWorkspaceContentState(root, contentDirectoryPath) {
  const statePath = path6.join(contentDirectoryPath, "state.md");
...
function inferEntryType(input) {
  ...
  if (segments[0] === chapterRoot && input.relativePath.endsWith(".md")) {
    if (path6.basename(input.relativePath).toLowerCase() === "index.md" && segments.length === 3) {
      return "volume";
    }
    return "chapter";
  }
...
function resolveWorkspaceIcon(input) {
  ...
  const baseName = path6.basename(input.relativePath.replace(/\/$/, ""));
  if (input.isDirectory && input.config.directories[baseName]) {
    return input.config.directories[baseName] ?? null;
  }
  const extension = path6.extname(input.absolutePath).toLowerCase();
...
async function readWorkspaceIconConfig(root) {
  const configPath = path6.join(root, ".nbook", "icons.json");
...
function parseReferenceLink(raw3) {
  const matched = /^\[([^\]]+)\]\((?:@)?([a-z]+):\/\/([^)]+)\)$/.exec(raw3);
  if (!matched) {
    return null;
  }
...
function extractInlineReferences(text) {
  ...
  for (const matched of text.matchAll(STANDARD_MARKDOWN_REFERENCE_PATTERN)) {
    ...
    const raw3 = matched[0] ?? "";
    if (matchedIndex < 0 || !raw3) {
      continue;
    }
    const parsedReference = parseReferenceLink(raw3);
    if (!parsedReference) {
      throwContentBadRequest(`\u975E\u6CD5 inline \u5F15\u7528\uFF1A${raw3}`);
    }
    tokens.push({
...
import { dirname as dirname2, isAbsolute, join as join5, posix, relative as relative2, resolve as resolve2 } from "node:path";
...
function assertAllowedWatchPath(path7, label) {
  if (!path7) {
    return;
  }
  if (!["ctx.session", "ctx.input", "ctx.runtime", "ctx.workspace"].some((prefix) => path7 === prefix || path7.startsWith(`${prefix}.`))) {
    throw new Error(`${label} \u53EA\u80FD\u4ECE ctx.session\u3001ctx.input\u3001ctx.runtime\u3001ctx.workspace \u5F00\u59CB\uFF1A${path7}`);
  }
}
function readPath(context, path7) {
  const customStatePrefix = "ctx.session.customState.";
  if (path7.startsWith(customStatePrefix)) {
    const customPath = path7.slice(customStatePrefix.length);
  ...
  const rootKey = Object.keys(roots).find((key) => path7 === key || path7.startsWith(`${key}.`));
  if (!rootKey) {
    return void 0;
  }
  const rest = path7 === rootKey ? [] : path7.slice(rootKey.length + 1).split(".");
...
var WriterOutputSchema = Type2.Object({
  summary: Type2.String({ description: "\u7EA6 100 \u5B57\u5199\u4F5C\u6458\u8981\uFF0C\u8BF4\u660E\u65F6\u95F4\u3001\u5730\u70B9\u3001\u53C2\u4E0E\u89D2\u8272\u3001\u5173\u952E\u52A8\u4F5C\u3001\u5173\u7CFB\u53D8\u5316\u548C\u4F0F\u7B14/\u72B6\u6001\u53D8\u5316\u3002" }),
...
var USER_ASSETS_WORKSPACE_ROOT = path.posix.join(WORKSPACE_CONTAINER_ROOT, ".nbook");
var USER_NBOOK_ROOT = path.posix.join(WORKSPACE_CONTAINER_ROOT, ".nbook");
var SYSTEM_WORKSPACE_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../assets/workspace");
var SYSTEM_NBOOK_ROOT = path.join(SYSTEM_WORKSPACE_ROOT, ".nbook");
var SYSTEM_PROFILE_ROOT = path.join(SYSTEM_NBOOK_ROOT, "agent", "profiles");
var USER_PROFILE_ROOT = path.resolve(process.cwd(), USER_NBOOK_ROOT, "agent", "profiles");
var SYSTEM_PROFILE_METADATA_PATH = path.join(SYSTEM_PROFILE_ROOT, ".system-profile-metadata.json");
var USER_PROFILE_SYNC_STATE_PATH = path.join(USER_PROFILE_ROOT, ".profile-sync-state.json");
var NOVEL_DIRECTORY_TEMPLATE_ROOT = path.join(SYSTEM_NBOOK_ROOT, "templates", "novel-directory-templates");
var USER_NOVEL_DIRECTORY_TEMPLATE_ROOT = path.resolve(process.cwd(), USER_ASSETS_WORKSPACE_ROOT, "templates", "novel-directory-templates");
...
// server/assets/asset-resolver.ts
var SYSTEM_ASSETS_ROOT = path2.resolve(path2.dirname(fileURLToPath2(import.meta.url)), "../../assets/workspace/.nbook");
var AssetResolver = class {
   ...
   */
  get userRoot() {
    return path2.resolve(this.workspaceRoot, USER_ASSETS_WORKSPACE_ROOT);
  }
  /**
  ...
  async resolveFile(relativePath) {
    ...
    const userPath = path2.join(this.userRoot, normalizedPath);
    ...
    const systemPath = path2.join(this.systemRoot, normalizedPath);
  ...
  async readFile(relativePath) {
    ...
    return fs2.readFile(resolved.absolutePath, "utf-8");
  }
  /**
  ...
  async listDirectories(relativePath) {
    ...
    await this.appendDirectories(directoriesByName, path2.join(this.systemRoot, normalizedPath), normalizedPath, "system");
    await ensureUserAssetsWorkspaceRoot();
    await this.appendDirectories(directoriesByName, path2.join(this.userRoot, normalizedPath), normalizedPath, "user");
  ...
  async copyMergedDirectory(relativePath, targetRoot) {
    ...
    const systemPath = path2.join(this.systemRoot, normalizedPath);
    const userPath = path2.join(this.userRoot, normalizedPath);
    const mergedRoot = await fs2.mkdtemp(path2.join(os.tmpdir(), "nbook-asset-merge-"));
    try {
      if (await isDirectory(systemPath)) {
        await fs2.cp(systemPath, mergedRoot, {
          recursive: true,
          force: true,
          errorOnExist: false
        });
      ...
      if (await isDirectory(userPath)) {
        await fs2.cp(userPath, mergedRoot, {
          recursive: true,
          force: true,
          errorOnExist: false
        });
      }
      await fs2.cp(mergedRoot, targetRoot, {
        recursive: true,
        force: false,
        errorOnExist: false
      });
    } finally {
      await fs2.rm(mergedRoot, { recursive: true, force: true });
    }
  }
  /**
   * 追加目录下的一级目录。
  ...
  async appendDirectories(directoriesByName, root, relativeRoot, source) {
    ...
    for (const entry of entries) {
      ...
      const childRelativePath = path2.join(relativeRoot, entry.name);
      directoriesByName.set(entry.name, {
        name: entry.name,
        absolutePath: path2.join(root, entry.name),
...
function normalizeAssetRelativePath(relativePath) {
  const normalizedPath = path2.normalize(relativePath).replace(/^([/\\])+/, "");
  if (normalizedPath === ".." || normalizedPath.startsWith(`..${path2.sep}`)) {
...
async function isDirectory(directoryPath) {
  try {
    return (await fs2.stat(directoryPath)).isDirectory();
  } catch (error) {
    if (isMissingPathError(error)) {
      return false;
    }
...
var WRITING_REFERENCE_DIR_CANDIDATES = [
  path3.join(process.cwd(), "server", "agent-v2", "profiles", "builtin", "writing-references"),
  path3.join(path3.dirname(fileURLToPath3(import.meta.url)), "builtin", "writing-references"),
  path3.join(assetResolver.systemRoot, "agent", "profiles", "builtin", "writing-references"),
  path3.join(assetResolver.userRoot, "agent", "profiles", "builtin", "writing-references")
];
...
async function loadWritingReferencePresets(candidates = WRITING_REFERENCE_DIR_CANDIDATES) {
  ...
  for (const referenceFile of referenceFiles) {
    const content = await fs3.readFile(referenceFile.absolutePath, "utf-8");
    ...
    references.push({
      ...parsed.metadata,
      sourceFile: path3.relative(process.cwd(), referenceFile.absolutePath).split(path3.sep).join("/"),
      content: parsed.body
    });
  }
    ...
    for (const entry of entries) {
      if (entry.isFile() && entry.name.endsWith(".md")) {
        filesByName.set(entry.name, {
          name: entry.name,
          absolutePath: path3.join(candidate, entry.name)
        });
      }
    }
  }
...
async function readOptionalDirectory(directoryPath) {
  try {
    return await fs3.readdir(directoryPath, { withFileTypes: true });
  } catch (error) {
...
var WRITING_STYLE_DIR_CANDIDATES = [
  path4.join(process.cwd(), "server", "agent-v2", "profiles", "builtin", "writing-styles"),
  path4.join(path4.dirname(fileURLToPath4(import.meta.url)), "builtin", "writing-styles"),
  path4.join(assetResolver.systemRoot, "agent", "profiles", "builtin", "writing-styles"),
  path4.join(assetResolver.userRoot, "agent", "profiles", "builtin", "writing-styles")
];
...
async function loadWritingStylePresets(candidates = WRITING_STYLE_DIR_CANDIDATES) {
  ...
  for (const styleFile of styleFiles) {
    const content = await fs4.readFile(styleFile.absolutePath, "utf-8");
    ...
    styles.push({
      ...parsed.metadata,
      sourceFile: path4.relative(process.cwd(), styleFile.absolutePath).split(path4.sep).join("/"),
      content: parsed.body
    });
  }
...
async function buildWriterPrompt(ctx) {
  ...
  return /* @__PURE__ */ jsxs(ProfilePrompt, { children: [
    ...
    /* @__PURE__ */ jsx(ModelContext, { children: /* @__PURE__ */ jsx(Message, { children: renderInputContext(ctx, { plotPointsText, lorebookText }) }) }),
...
async function buildPlotPointsText(novelIdText, sceneIds) {
...
async function readContentNodeFiles(workspaceRoot, entry) {
  ...
  const statePath = join5(dirname2(indexPath), "state.md");
  let indexRaw = "";
  try {
...
function renderPlotPointBlock(index, sceneIdText, thread, scene, chapterPlot) {
  return [
...
function renderKeyValueLines(entries) {
  const lines = entries.filter(([, value]) => typeof value === "string" && value.trim()).map(([key, value]) => `${key}: ${value}`);
  return lines.length > 0 ? lines.join("\n") : "\u7A7A";
}
function resolveWriterNovelId(novelIdText) {

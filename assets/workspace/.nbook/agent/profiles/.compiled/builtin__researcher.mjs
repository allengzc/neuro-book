...
var InputSchema = ResearcherInputSchema;
var allowedToolKeys = ["web_search", "web_fetch"];
var researcher_profile_default = defineAgentProfile({
  ...
  context(ctx) {
    return /* @__PURE__ */ jsxs(ProfilePrompt, { children: [
      /* @__PURE__ */ jsx(System, { children: RESEARCHER_SYSTEM_PROMPT }),
      ...
      /* @__PURE__ */ jsx(AppendingSet, { children: /* @__PURE__ */ jsx(Message, { children: renderResearchBrief(ctx.input) }) })
    ] });
  }
});
...
var RESEARCHER_SYSTEM_PROMPT = profileText`
        ...
        - web_search 只返回搜索结果摘要。需要核对细节、引用原文信息或判断页面质量时，对高价值 URL 调用 web_fetch。
        - 简单问题通常 1 次 web_search 加 0 到 2 次 web_fetch 就足够。不要为了显得严谨而堆搜索、堆来源或把短任务升级成完整调研。
        ...
        - 默认先用 web_search 找来源，再用 web_fetch 读取少量最相关页面；不要为了堆数量抓取大量低价值页面。

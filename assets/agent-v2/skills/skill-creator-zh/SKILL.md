...
description: 用于在 Neuro Book 仓库中创建、改造或维护 skill。适用于新增 `assets/agent/skills/*`、重写现有 skill、整理 skill 目录结构、补充脚本或参考资料、修正 skill frontmatter 与触发描述等场景。
---
...
1. 服务端扫描 `assets/agent/skills/*/SKILL.md`，兼容旧文件名 `skill.md`
2. catalog 只读取 frontmatter 里的 `name` 和 `description`
3. 模型先看到 catalog 元数据
4. 真正需要使用 skill 时，再通过 `read_file(location)` 读取完整 skill 正文
...
- `assets/agent/skills/shuangwen`
- `assets/agent/skills/worldbuilding-helper`
...
3. 判断需要哪些可复用资源：
   ...
   - 详细规范放 `references/`
   - 输出模板放 `assets/`
4. 在 `assets/agent/skills/<folder>/` 下创建 skill
5. 写 frontmatter 与正文
...
如果当前 agent 有 `execute_shell` 能力，可以使用本目录下的脚本加速：

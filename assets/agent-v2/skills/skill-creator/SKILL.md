---
name: skill-creator
description: Create or update a Neuro Book skill under assets/agent/skills. Use this when you need to design a new skill, refactor an existing skill, tighten trigger descriptions, reorganize bundled resources, or add optional helper scripts and references for the project skill system.
---
...
This project currently discovers skills by scanning `assets/agent/skills/*/SKILL.md` (or legacy `skill.md`) and reading only the frontmatter fields `name` and `description`. The skill body is loaded later with `read_file(...)` only when the model decides the skill is relevant.
...
Each skill is a folder under `assets/agent/skills/<folder>`.
...
3. Create or update the skill folder under `assets/agent/skills`.
...
- the skill folder lives under `assets/agent/skills`
- `SKILL.md` exists

...
- profile normalize 会收集所有 sidecar 的 `sidecarDataSchema`，并把它们作为 stable union 渲染到 `report_result.sidecar_data` 的 provider-visible schema。
- sidecar 运行期按当前 active sidecar 精确校验 `sidecar_data`；校验失败发生在 `report_result` tool execution 阶段，返回模型可见 error toolResult，并允许同一 run 自我修正。

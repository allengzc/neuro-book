...
- **声明了非空 `default`**：创建 subject 时（首次 `world.writeSlice` 在 patch 上声明 `type` 自动触发），会在该 subject 的起始 instant 生成或追加一组 `kind=init` 的 `replace` patch，把初值写进切面。这样 reduce 从切面序列即可拿到已声明的初值，**切面序列自包含**，不需要「reduce 时回查 schema 兜底」的特殊逻辑。
...
- **subject type 名**：不能为空，不能含空白，不能含括号。这套规则同时用于 schema type 声明、运行时 `world.writeSlice` 首写时 patch 上的 `type` 字段（触发自动创建）、`execute_world` 中 `world.list(type)` 查询，以及 `ref(<type>)` 校验。`ref(<type>)` 必须指向同一 schema 已声明的 subject type，悬空 ref 在 schema 加载阶段就报错。

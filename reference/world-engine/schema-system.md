...
- **声明了非空 `default`**：创建 subject 时（首次 `write_world_slice` 在 patch 上声明 `type` 自动触发），会在该 subject 的起始 instant 生成或追加一组 `kind=init` 的 `replace` patch，把初值写进切面。这样 reduce 从切面序列即可拿到已声明的初值，**切面序列自包含**，不需要「reduce 时回查 schema 兜底」的特殊逻辑。
- **没有 default**：只注册 subject 身份，**不创建空切面**（保持「切面 = 状态变更」这条约束，不存在没有 patch 的切面）。

初始化追加的边界：自动追加**只允许落到同 instant 已有的 `kind=init` 切面**。如果该时刻已经有 `event` / `backstory` 等非 init 切面，引擎会拒绝自动追加（返回 409），要求换一个初始化时间 —— 避免 subject 创建悄悄改写普通事件切面。注意 Agent 工具层没有编辑已有切面的能力（`editSlice` 只在 Workbench / HTTP 层可用），只能改时间重写。
...
- **subject type 名**：不能为空，不能含空白，不能含括号。这套规则同时用于 schema type 声明、运行时 `write_world_slice` 首写时 patch 上的 `type` 字段（触发自动创建）、`execute_world_query` 中 `world.list(type)` 查询，以及 `ref(<type>)` 校验。`ref(<type>)` 必须指向同一 schema 已声明的 subject type，悬空 ref 在 schema 加载阶段就报错。
...
跑几条 patch 验证 op + reduce（时间一律用项目日历字符串，Agent 工具 / HTTP 公开入参禁止 raw `instant:<number>`）：
...
// 切面 @ "星辉历312年 5月5日 14:00"：艾莉娜受伤、捡到剑、装上、对师门改观
{
    ...
    "patches": [
        ...
        { "subjectId": "erina", "path": "/hp", "op": "increment", "value": -30, "summary": "受伤失去体力" },
        ...
        { "subjectId": "erina", "path": "/memory/师门", "op": "replace", "value": "怀疑", "summary": "更新师门看法" },
        // list 追加：写一条经历
        { "subjectId": "erina", "path": "/events", "op": "append", "value": "在城北被伏击，捡到一把旧剑", "summary": "记录经历" }
    ]
}
```

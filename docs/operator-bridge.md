...
2. 用户希望的模式：默认 `ghcr`、`source` 源码挂载，还是免 Docker `native`。
...
NeuroBook 支持 Docker Compose 单机部署，也支持免 Docker native 部署。运行私有配置放在项目根目录 `.env`、Boot Config `config.yaml` 和 Global Config `workspace/.nbook/config.json`，部署生成物放在 `.deploy/`。

仓库跟踪的模板文件：

- `docker-compose.yml`：基础模板，默认只启动 `app`，数据库为 SQLite 文件库。
- `docker-compose.postgres.yml`：内置 Postgres override。
- `docker-compose.external-db.yml`：外部 Postgres override。
...
- `.env`：容器运行环境变量，例如端口、session password、`DATABASE_KIND`、`DATABASE_URL`，以及 Postgres 模式下的密码。
...
- `.deploy/docker-compose.generated.yml`：根据 `ghcr` 或 `source` 模式生成的 compose override；native 模式不使用该文件。
...
native 模式不使用 Docker。应用直接在宿主机项目目录中运行，读取同一份 `.env`、`config.yaml` 和 `workspace/.nbook/config.json`。
...
native 模式适合不能或不想安装 Docker 的机器。

特点：

- 宿主机 git clone 项目源码。
- 宿主机安装依赖、Prisma generate、Nuxt build 和数据库迁移。
- 服务用 `node .output/server/index.mjs` 启动。
- 部署脚本生成 `.deploy/start-native.sh` 或 `.deploy/start-native.ps1` 便于启动服务，但不生成 systemd/pm2 服务，不接管后台进程管理。
- 数据库默认 SQLite；也可填写外部 PostgreSQL URL。native 不负责安装或管理本机 PostgreSQL。
...
- `--dry-run --yes` 会展示将写入的文件路径、清理动作和将执行的命令；native 模式仍会探测本机命令是否存在，用来展示缺失工具和安装建议，但不会安装、构建、迁移、启动服务或写入部署文件。
...
如果 app 不可访问，Docker 模式先看容器是否反复重启，再看 app 日志；Postgres 模式再看 Postgres 日志。native 模式直接查看运行 `node .output/server/index.mjs` 的终端输出。
...
- `DATABASE_KIND`
- `DATABASE_URL`
- `POSTGRES_USER` / `POSTGRES_PASSWORD` / `POSTGRES_DB`（仅内置 Postgres 模式）
...
Postgres 模式下宿主机执行 Prisma 前没有加载 `.env`。SQLite 模式缺省使用 `file:./workspace/.nbook/neuro-book.sqlite`。

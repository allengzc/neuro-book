...
- **[集成前置·2026-07-07 可行性分析核实]（挂点全部存在且形状吻合，无架构性障碍）**：
  ...
  3. **生命周期挂点已就绪**：Task 92 建成 Project 资源统一注册表（`server/workspace-files/project-resources.ts`，删除/空闲/关停三路径），集成时把 `WorkspaceHistory` 注册为资源属主即可（close = `history.close()`）。

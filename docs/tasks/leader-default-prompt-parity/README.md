...
- 当前 v3 `leader.default` 已迁入部分 v2 语义，但仍偏压缩；缺少 v2 中更完整的 Task Management、内容节点引用分流、Plan Mode full/sparse/exit/reentry 细节和部分 workspace CLI 规则。
- 当前 v3 已有工具：`read`、`write`、`edit`、`apply_patch`、`bash`、`create_agent`、`invoke_agent`、`get_agent`、`get_session`、`detach_agent`、`request_user_input`、`enter_plan_mode`、`exit_plan_mode`、`skill`、`report_result`。
- 当前 v3 尚未注册 task 工具、plot 工具、SQL 工具、writer profile、retrieval profile。
...
1. 清理当前 v3 leader DSL 表达
   ...
   - 把 `<SkillCatalog text={renderAgentCatalog(ctx)} />` 改为 `<SkillCatalog />`，默认由 DSL runtime 拼接可用 agent catalog。
...
5. 迁移 writer / retrieval profile
   ...
   - writer 工具改为 v3 名：`read`、`write`、`edit`、`apply_patch`、`report_result`；不默认给 `bash` / `skill`。
   - retrieval 工具改为 v3 名：`bash`、`read`、`skill`、`report_result`。
...
10. 更新文档
   ...
   - 更新 `docs/tasks/leader-profile-v2-adaptation/README.md`，链接到本任务并说明 prompt parity pass。
   - 如实际长期状态变化，更新 `docs/tasks/pi-agent-harness-migration/README.md` 和 `PROJECT-STATUS.md`。
...
- `docs/tasks/leader-default-prompt-parity/README.md`
...
- 修正 catalog 语义：`SkillCatalog` 只展示 skills，新增 `AgentCatalog` 展示可创建/调用的 agent profiles 和 schema 摘要。
...
- `SkillCatalog` 已恢复 v2 语义：只渲染 `## Skill` 与 `## Available Skills`，数据来自 `.nbook/agent/skills` catalog，并支持 `when_to_use` frontmatter 摘要；v3 工具参数使用 `skillKey`，因此 catalog 同时展示 key/name，并要求调用 `skill({ skillKey })`。
- 新增 TSX DSL string fragment `<AgentCatalog />`：渲染 `## Available Agents`，展示 profile key、name、description、allowed tools、InputSchema / OutputSchema 的字段级摘要。

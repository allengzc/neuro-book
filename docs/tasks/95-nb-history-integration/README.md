...
> **状态：已实施（2026-07-09；2026-07-10 经 Task 102 完成 Composer / 安全 diff / profile-controlled notice 演进）**——自动化门禁全绿，剩浏览器手动走查（按惯例交用户）。实施计划见 `~/.claude/plans/1-writer-tidy-hamster.md`（批准版），本 README 记录执行结果与偏差。
...
- 公开 hash-only snapshot API 已删除。新 `/api/workspace-history/diff` 必须用当前 inbox path 授权；`.env*`、凭据、私钥/证书、`.ssh` 等敏感路径在服务端调用 diff 前阻断，inline 大 diff 只返回统计。
...
- 验证：Workspace History 22/22、Task 102 聚焦 48/48、Agent tools 147/147、全仓 typecheck 退出 0。

详见 [Task 102](../102-agent-change-inbox-and-prompt-order/README.md)。

## TODO / Follow-ups

- [ ] 浏览器走查（用户）：Composer 默认收起与小 diff、`.env` 服务端阻断、leader(full) / writer(minimal) / inline(0) notice、完整收件箱 accept/revert、`.nbook/history.sqlite` 落盘。

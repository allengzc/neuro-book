...
- 目标合同：
    - URL/API/前端状态优先使用 `projectSlug` 或 `projectPath`。
    - Project Workspace 元数据文件使用 `project.yaml`，用 `kind: novel` 表达项目类型。
    - `/api/novels` 可以在过渡期保留路由名，但返回值必须来自 Project Workspace metadata，而不是 `Novel` 表。
...
- 优先把外部可见合同从 `novelId` 迁到 `projectSlug` / `projectPath`。
...
- Phase 2：删除 `Novel` 表和 DB 外键依赖。
    ...
    - 将 `Story.novelId` 迁移为 `Story.projectPath` 或等价 Project identity 字段。
    - migrations 同步处理 SQLite/Postgres fresh DB；不做旧 Postgres 自动导入。
- Phase 3：迁移 API / 前端 contract。
    - `/api/novels` 列表改为扫描 Project Workspace + `project.yaml`。

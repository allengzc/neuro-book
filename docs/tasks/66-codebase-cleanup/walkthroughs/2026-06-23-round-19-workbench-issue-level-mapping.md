# Round 19 - Workbench Issue Level Mapping

## Background

继续 Task 66 的自主清理。本轮仍遵守当前约束：

- bug 不顺手修，先记录。
- 复杂问题先记录。
- 符号命名和文件结构问题先记录，等待审批。

Round 18 已把 draft surface auto-open 规则下沉到 `world-engine-workbench-real.ts`。本轮继续寻找重复但简单的纯规则。

## Evidence

- 当前 worktree 仍有 Task 66 相关未提交变更；另有 `AGENTS.md`、`assets/workspace/.nbook/agent/profiles/builtin/writer.profile.tsx`、`assets/workspace/.nbook/templates/project-directory-templates/world-engine/index.md`、`docs/frontend-notification-channels.md` 等非本轮文件已修改或新增，本轮不触碰。
- issue code 到 A/E 等级的映射原本重复出现在：
  - `WorldEngineWorkbenchDialog.vue`
  - `WorldEngineWorkbenchPreviewSliceCard.vue`
  - `WorldEngineWorkbenchPreviewMutationEditor.vue`
- 重复规则一致：`base-shifted` 和 `masked` 是 A，其它 issue code 是 E。
- Mutation Editor 额外有 `manual-focus` 分支，这不是 World Engine issue code，需要保留在本地 adapter。

## Assessment

候选问题：

1. **issue code 到 A/E 的重复映射**
   - 本轮采用。
   - Module 有轻量 Depth：统一 A/E 等级规则，避免三个组件未来漂移。
   - Interface 小：输入 `WorldIssueDto["code"]`，输出 `"A" | "E"`。
   - Locality 更好：共享规则在 util，组件本地 `issueLevel` 继续作为 UI adapter。

2. **issue status label / class 重复**
   - 本轮不做。
   - status 文案和 class 与组件展示差异更强，属于 UI 表现层规则，先不顺手扩大。

## Plan

- 在 `app/utils/world-engine-workbench-real.ts` 新增：
  - `WorldWorkbenchIssueLevel`。
  - `worldWorkbenchIssueLevel`。
- `WorldEngineWorkbenchDialog.vue`、`WorldEngineWorkbenchPreviewSliceCard.vue`、`WorldEngineWorkbenchPreviewMutationEditor.vue` 的本地 `issueLevel` 保留，但内部调用共享 util。
- 在 `app/utils/world-engine-workbench-real.test.ts` 增加行为测试。
- 更新 `app/utils/world-engine-workbench-preview.test.ts` 的静态断言，让它确认组件调用共享 util，而不是要求组件里出现映射字面量。

## Changes

- `app/utils/world-engine-workbench-real.ts`
  - 新增 `worldWorkbenchIssueLevel`。
- `app/components/novel-ide/world-engine/WorldEngineWorkbenchDialog.vue`
  - 本地 `issueLevel` 调用共享 util。
- `app/components/novel-ide/world-engine/workbench-preview/WorldEngineWorkbenchPreviewSliceCard.vue`
  - 本地 `issueLevel` 调用共享 util。
- `app/components/novel-ide/world-engine/workbench-preview/WorldEngineWorkbenchPreviewMutationEditor.vue`
  - 保留 `manual-focus` 分支；真实 issue code 调用共享 util。
- `app/utils/world-engine-workbench-real.test.ts`
  - 新增“将 issue code 映射成 A/E 等级”测试。
- `app/utils/world-engine-workbench-preview.test.ts`
  - 更新静态断言。
- `docs/tasks/66-codebase-cleanup/README.md`
  - 更新 Round 19 索引、热点行数、最新验证结果。
- `PROJECT-STATUS.md`
  - 更新 Task 66 当前说明。

## Verification

运行：

```powershell
bunx vitest run app/utils/world-engine-ide-entry.test.ts app/utils/world-engine-workbench-real.test.ts app/utils/world-engine-workbench-preview.test.ts
```

结果：

- 3 files passed。
- 19 tests passed。

## No Behavior Drift Check

- `base-shifted` 仍映射为 A。
- `masked` 仍映射为 A。
- `broken-relative` / `dangling-ref` 仍映射为 E。
- Mutation Editor 的 `manual-focus` 仍映射为 `manual`，未交给共享 util。
- issue class、status label、status class、模板调用方式均未改变。

## Bugs / Complex Issues

- Bug：本轮未发现业务 bug。
- 复杂问题记录：issue status label / class 是否统一属于 UI 表现规则，本轮不扩大处理。
- 命名 / 文件结构：没有重命名旧符号，没有拆文件；Round 13 的待审批项仍保持。

## Current Measurements

- `app/components/novel-ide/world-engine/WorldEngineWorkbenchDialog.vue`：约 2153 行。
- `app/utils/world-engine-workbench-real.ts`：约 889 行。
- `app/utils/world-engine-ide-entry.test.ts`：约 1225 行。
- `app/utils/world-engine-workbench-real.test.ts`：约 811 行。
- `app/utils/world-engine-workbench-preview.test.ts`：约 1140 行。

## Follow-ups

- issue status label / class 重复先不处理，避免把 UI 表现层规则一次性拉大。
- 继续寻找简单纯规则下沉点。
- 命名和文件结构候选仍等待用户审批。

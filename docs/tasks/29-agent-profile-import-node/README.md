...
- `spec/` 和 `docs/` 可以给 agent 使用，构建产物应该包含这些目录。
- 后续整理 `spec/` 文档，让 profile 可以引用稳定规范，而不是在 prompt 中内嵌大量重复说明。

## Goal

新增 Profile DSL `Import` 节点，使 profile 作者可以显式加载共享文档上下文，减少 builtin profile prompt 中重复、易漂移的协议说明。第一版以 UTF-8 文本导入为目标，验证面包括 Profile DSL 渲染测试、profile workbench parser、profile check、system build 产物包含 `AGENTS.md`、`spec/` 与 `docs/`，并把 `leader.default` 中一部分共享项目协议迁移为 `<Import />` 引用。

成功标准：

- Profile DSL 支持类似 `<Import path="spec/content/lorebook-information-control.md" />` 的节点。
...
- 构建 / 发布产物包含 profile 可能引用的 `AGENTS.md`、`spec/`、`docs/`。
...
- `leader.default` 已迁移为通过 `HistorySet > Message > Import` 加载三份共享规范：`spec/agent/leader-default.md`、`spec/content/markdown-dialect.md` 和 `spec/agent/neurobook-project-guide.md`。
- 构建脚本已把 repo 根 `AGENTS.md`、`spec/`、`docs/` 明确纳入 compiled runtime 可引用资源。
- Profile Workbench 已能识别、展示和拖入 `Import` 节点。
- `spec/agent/README.md` 和 `spec/content/README.md` 已成为 Agent 阅读稳定规范的入口。
...
- 已新增稳定规范 `spec/agent/profile-import.md`，并整理 `spec/agent/profile-guide.md`、`spec/agent/context.md` 和 `spec/agent/neurobook-project-guide.md`。
...
- 已从 `leader.default` prompt 中继续抽出共享操作协议，新增 `spec/agent/leader-default.md`，覆盖工具使用、Task Management、多 Agent 协作、writer / retrieval / researcher / RP 调度、SQL、Plan Mode 和 Skills。
- 已从 `leader.default` 的 Markdown 扩展说明中抽出 `spec/content/markdown-dialect.md`，覆盖 workspace links、inline comment、mark、文本颜色、上下标和 align。
- 已新增 `spec/agent/README.md` 与 `spec/content/README.md`，让 Agent 先从稳定规范入口阅读，而不是优先读任务 walkthrough。
...
- 已删除 active 旧工具表 `docs/modules/agent/tools-reference.md`；它仍在描述 v2 工具名，当前合同改由 `spec/agent/leader-default.md` 和迁移任务记录承担。
...
- `spec/` 和 `docs/` 是 agent 可用的共享知识层。
  - `spec/` 放稳定规范和实现契约，适合被 profile 导入。
  - `docs/` 放任务、研究和草案，适合按需导入；不要把未稳定草案默认长期注入所有 profile。
- 构建产物必须包含 `AGENTS.md`、`spec/`、`docs/` 或至少包含 profile import 允许引用的子集。
...
<HistorySet>
    <Message>
        <Import path="spec/content/lorebook-information-control.md" />
    </Message>
</HistorySet>
```
...
```tsx
<Import
    path="spec/content/lorebook-information-control.md"
...
```text
[Import truncated: spec/content/lorebook-information-control.md maxBytes=12000]
```spec/content/lorebook-information-control.md
...
```
```
...
- `spec/agent/profile-import.md`：`Import` 节点的 API、放置位置、安全边界和 build contract。
- `spec/agent/profile-guide.md`：active DSL 指南，包含 `Import` 的推荐写法和 checklist。
- `spec/agent/context.md`：active `ProfilePrompt` 拆分规则，说明 `Import` 作为 string fragment 的上下文位置。
- `spec/agent/README.md`：Agent 稳定规范入口。
- `spec/agent/leader-default.md`：默认 Leader 可共享的操作协议。
- `spec/agent/neurobook-project-guide.md`：面向 agent profile 共享导入的 NeuroBook Project Workspace 操作指南。
- `spec/content/README.md`：内容规范入口。
- `spec/content/markdown-dialect.md`：NeuroBook Markdown 扩展格式。
- `spec/README.md`：补充 Agent / Content spec 入口与关键共享规范索引。

入口文档现已调整为：

- Agent prompt / profile / 工具协作 / workspace 文件语义：先读 `spec/agent/README.md`、`spec/agent/leader-default.md`、`spec/agent/neurobook-project-guide.md`、`spec/agent/profile-import.md`。
- 内容节点 / lorebook / simulation / Markdown 扩展：先读 `spec/content/README.md`。
...
- `spec/README.md`
- `spec/agent/README.md`
- `spec/agent/leader-default.md`
- `spec/agent/profile-import.md`
- `spec/agent/profile-guide.md`
- `spec/agent/context.md`
- `spec/agent/neurobook-project-guide.md`
- `spec/content/README.md`
- `spec/content/markdown-dialect.md`
...
- 文档引用审计：
  - `rg -n "tools-reference\\.md|docs/modules/agent/tools-reference|agent/tools-reference" README.md docs spec PROJECT-STATUS.md`：仅剩“旧文档已下线”的说明。
  - `rg -n "# 工具使用|# Task Management|# 多 Agent|# Markdown 扩展|## Anatomy Lorebook|## Anatomy Manuscript|## Anatomy Plot System|# Shell commands|read_file|write_file|execute_shell" assets/workspace/.nbook/agent/profiles/builtin/leader.default.profile.tsx workspace/.nbook/agent/profiles/builtin/leader.default.profile.tsx server/agent/profiles/leader-assets-profile.test.ts spec/agent/leader-default.md spec/content/markdown-dialect.md spec/agent/neurobook-project-guide.md`：profile 源码中无旧长段落或 v2 工具名；仅 spec/test 中保留预期标题和负断言。
- 未做浏览器验证；Profile Workbench 的本轮补齐只做代码级接入和窄测试覆盖。
- 全量 `bun run typecheck` 此前仍受既有无关类型问题阻塞，本任务未修改那些问题。

## TODO / Follow-ups

- 继续观察 `leader.default` 中是否还有适合迁移到 `spec/agent/neurobook-project-guide.md` 的共享项目协议段。

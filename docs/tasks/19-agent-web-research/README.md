...
- 进入技术选型阶段，评估 Exa、Tavily、Firecrawl、Brave Search、服务端自建 fetch，以及 HTML 解析/清洗库。
...
- 已进入技术选型阶段，候选包括：
  ...
  - Brave Search API：独立搜索索引，适合纯搜索结果层；fetch/清洗需要 NeuroBook 自己补。
...
| 方案 | 优点 | 缺点 | 适配判断 |
| --- | --- | --- | --- |
| Tavily | Search API 明确面向 LLM 场景；支持 include/exclude domain 过滤、时间过滤、结果摘要，也可选返回清洗后的 raw content。 | 外部付费 provider；advanced / auto 参数可能增加成本；provider-specific 参数应藏在 adapter 后面。 | 最适合作为 agent-facing search 的第一版默认方案。 |
...
| Brave Search API | 独立搜索索引；支持 freshness、country/language targeting、extra snippets、搜索操作符和分页。 | 主要提供搜索结果层；NeuroBook 仍要自己 fetch 和 clean 页面。 | 当我们需要更可控的低层搜索后端时适合接入。 |
| 通过公共搜索页面做本地搜索 | 如果存在稳定允许的 endpoint，会便宜且简单。 | 抓取搜索结果页很脆弱，也容易触碰服务政策边界。 | 不推荐作为第一版实现。 |

初步建议：

- 第一版 `web_search` 使用 Tavily adapter。
- 保留 provider interface，让 Exa / Brave / Firecrawl 后续能在不改变工具 schema 的情况下接入。
...
- fallback 路径做成可配置 provider fetch：如果搜索 provider 已配置，优先 Tavily 或 Exa；如果抓取质量成为主要问题，再接 Firecrawl。
...
- 起步使用 `jsdom` + `@mozilla/readability` + `turndown`。
...
- 决定第一版 `web_fetch` fallback provider：当前倾向 local first，Tavily/Exa fallback，Firecrawl 留作 scrape-heavy fallback。

...
- 补齐 harness 队列状态：
  ...
  - 新增 `steerQueue`，在当前 assistant turn + tool results 落盘后进入 safe point；如果 ReAct loop 还有下一轮模型调用，则在这个可引导点一次性 drain 当前所有 pending steer，并在下一次模型调用前注入。
  - 如果 assistant 没有 tool calls，当前 ReAct loop 在该 assistant message 后结束；此时 queued steer 和 queued followUp 的执行时机等价，都作为后续消息重新开启下一轮 loop，并重新执行 profile prepare / compaction / model resolve。
...
- 第一版固定 drain mode：steer 在同一个可引导点使用 all drain，一次性消费所有 pending steer；followUp 在 ReAct loop 结束后 one-at-a-time drain，一次只消费一条并开启下一轮 loop。
- steer / followUp 没有全局优先级；只有当当前 ReAct loop 仍会继续时，steer 才比 followUp 更早影响下一次模型调用。若当前 assistant 无 tool calls、loop 已结束，二者都按后续消息启动下一轮 loop，其中退化成后续 loop 的 steer 必须重新 prepare。
...
- followUp 只在当前 ReAct loop 结束后自动重新开启一个新 loop；不会在仍有 tool calls 的当前 loop 中插队。
...
- Harness 测试：运行中 enqueue steer 后，在 tool result 后、下一次模型调用前注入；followUp 在 run 结束 safe point 后继续；abort 清空两个队列。

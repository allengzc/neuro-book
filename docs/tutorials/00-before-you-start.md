...
进入设置页，配置模型 Provider、API Key 和默认模型。部署脚本不会替你保存 API Key；Agent 能不能工作，取决于这里是否配置成功。
...
如果你已经有项目，先确认页面当前选中的项目就是你要写的那一本。后续 Agent 读写文件、调用 writer、导入角色卡、进入 RP，都会围绕当前 Project Workspace 工作。

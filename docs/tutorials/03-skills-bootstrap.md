...
如果你还只有一个模糊想法，先让 Agent 使用 `小说灵感探索流程`。

可以这样说：

```text
使用“小说灵感探索流程”和我讨论这个想法：失忆检票员、夜行列车、梦境城市。目标是得到一个可以继续开发的故事雏形。
```
...
当故事雏形基本稳定后，使用 `小说初始化流程`。

```text
使用“小说初始化流程”为当前 Project Workspace 初始化这本书。请先列出你会创建或更新的文件，再执行。
```
...
接着使用 `世界书初始化流程`。世界书不是百科全书，它是 AI 的创作说明书。开局只需要足够支撑前三章，不要一口气写完整个宇宙。
...
当主角和关键人物开始清晰后，使用 `角色设计流程`。

```text
使用“角色设计流程”深化主角和第一位重要配角。请把稳定设定写入 lorebook，暂不进入 RP。
```
...
| Skill | 适合什么时候用 |
| --- | --- |
| `小说灵感探索流程` | 只有灵感，还没有故事骨架。 |
| `小说初始化流程` | 要把项目从空目录变成可写状态。 |
| `世界书初始化流程` | 要建立开篇可用的 lorebook。 |
| `角色设计流程` | 要深化主角、配角、反派或势力代表。 |
| `开局剧情设计` | 要把设定转成前三章剧情。 |
| `剧情规划流程` | 要整理中长期剧情线。 |

...
Profile 定义一个 Agent 的身份、输入输出、工具权限和上下文结构。NeuroBook v3 中，profile 就是 agent 类型：创建 `leader.default`、`writer`、`retrieval` 或 `leader.rp`，本质上都是创建某个 profile 的 session。

普通用户通常只需要选择 profile；profile 作者则需要理解 TSX Profile DSL、工具权限、输入输出 schema 和编译流程。
...
- `context(ctx)`：用 TSX DSL 生成 system、history、model context 和 appending context。

需要结构化结果时，profile 还会定义 `outputSchema`，并通过 `report_result` 返回。

...
Leader profile 负责理解用户意图、选择流程、调用 Skill 和协调其他 agent。普通写作入口是 `leader.default`；世界模拟 / RP 入口是 `leader.rp`；用户资产维护入口是 `leader.assets`。
...
`leader.rp` 是世界模拟 / RP 的 simulator leader。它读取 `simulation/config.yaml`、`simulation/cast.yaml`、`simulation/simulator.md` 和 `simulation/writer.md`，并调度 `rp.actor` 与 `rp.writer`。

一次 RP Tick 通常是：

```text
用户行动 -> leader.rp 裁决和调度 -> rp.actor 角色反应 -> leader.rp 世界推进 -> rp.writer 渲染可见正文
```
